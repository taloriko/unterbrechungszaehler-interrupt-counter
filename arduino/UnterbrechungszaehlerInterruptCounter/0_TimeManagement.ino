// 0_TimeManagement.ino
// Time hierarchy/status UI. The filename intentionally sorts before the other
// UI extensions so its root handler can load all extension scripts.
...
public:
  TimeManagementRegistrar() {
    server.on("/", HTTP_GET, tmServeIndex);
