...
// Aggregates kept in RAM. About 21 kB total.
static uint16_t longTermWeekdayHour[7][24];
static uint16_t longTermMonthWeek[LONGTERM_CACHE_YEARS][12][52];
...
static bool longTermRebuildAggregates() {
  ...
  for (uint32_t i = 0; i < longTermHeader.count; i++) {
    ...
    if (t.tm_mon >= 0 && t.tm_mon < 12) {
      longTermInc16(longTermYearMonth[yi][t.tm_mon]);
    }
    ...
    if (week > 52) week = 52;
    if (t.tm_mon >= 0 && t.tm_mon < 12) {
      longTermInc16(longTermMonthWeek[yi][t.tm_mon][week - 1]);
    }
  }
  f.close();
...
static String longTermAggregateJson(int selectedYear) {
  ...
  for (int d = 0; d < 7; d++) {
    if (d) json += ',';
    longTermAppendUint16Array(json, longTermWeekdayHour[d], 24);
  }
  json += ']';
  ...
  for (int m = 0; m < 12; m++) {
    if (m) json += ',';
    longTermAppendUint16Array(json, longTermMonthWeek[selectedIndex][m], 52);
  }
  json += ']';
...
public:
  LongTermApiRegistrar() {
    server.on("/api/aggregate", HTTP_GET, []() {
      ...
      server.send(200, "application/json", longTermAggregateJson(year));
    });
  }
};
