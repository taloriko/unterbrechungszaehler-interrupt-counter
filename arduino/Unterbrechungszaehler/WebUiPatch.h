...
function init(){addWeek();addBeta();addMeters();addBaseExportMeta();addArchiveExport();normalizeStorageUi();hookTheme();hookFetch();fastButton('addBtn',1,'/api/add');fastButton('undoBtn',-1,'/api/delete-last');let language=$('languageSelect');if(language)language.addEventListener('change',()=>setTimeout(applyStaticUi,220));setTimeout(renameOverview,150);setInterval(isoWeekFromDevice,30000)}
init();
})();
</script>
)HTML";
