...
function tr(de,en,swg){let l=(localStorage.getItem('uic-lang')||'de').toLowerCase();return l==='en'?en:(l==='swg'?swg:de)}
...
function hardwareFor(name,d){let ok=tr('erkannt','detected','gfunda'),missing=tr('nicht erkannt','not detected','ned gfunda'),active=tr('aktiv','active','aktiv'),internal=tr('intern','internal','intern');switch(name){case'MainLoop':return'ESP32 · '+internal;case'Input':return'GPIO 27 · '+active;case'Mode':return'ESP32 · '+internal;case'Storage':return'LittleFS · '+active;case'Time':return'ESP32 RTC · '+active;case'RTC':return'DS3231 · '+(d.rtcPresent?ok:missing);case'Autark':return'GPIO 33 · '+active;case'Display':return'OLED · '+(d.displayPresent?ok:(d.displaySimulation?tr('Simulation','simulation','Simulation'):missing));case'LED':return'GPIO 2 · '+active;case'Network':return'WLAN · '+(d.wifi?tr('verbunden','connected','verbunda'):tr('offline','offline','offline'));case'Web':return'TCP/IP · '+active;case'Analytics':return'LittleFS · '+active;case'Sound':return'DY-SV17F · '+((d.sound||{}).present?ok:missing);case'Diagnostics':return'ESP32 · '+internal;default:return'-'}}
function cleanDetail(m){let x=String(m.detail||'-');if(['gpio','rings_ok','optional','wifi','hardware_lost'].includes(x))return'-';if(x==='serial')return'Serial 115200';if(x==='loop_cycle')return tr('Zyklus','cycle','Zyklus');if(x==='http_80')return'Port 80';if(x==='cache_ready')return tr('Cache bereit','cache ready','Cache bereit');if(x.startsWith('comm=')){let p={};x.split(';').forEach(v=>{let a=v.split('=');p[a[0]]=a.slice(1).join('=')});let out=[];out.push(p.valid==='yes'?tr('Zeit gültig','time valid','Zeit passt'):tr('Zeit ungültig','time invalid','Zeit passt ned'));out.push(p.osf==='clear'?'OSF OK':'OSF '+(p.osf||'?'));if(p.temp)out.push(p.temp+' °C');if(p.age)out.push(tr('Prüfung ','check ','Prüfung ')+Math.round(Number(p.age)/1000)+' s');return out.join(' · ')}return x.replaceAll('_',' ')}
function ensureHardwareHeader(){let table=document.querySelector('.v2WdTable'),head=table&&table.tHead&&table.tHead.rows[0];if(!head)return;if(!head.querySelector('[data-v212-hardware]')){let th=document.createElement('th');th.dataset.v212Hardware='1';th.textContent=tr('Hardware','Hardware','Hardware');head.insertBefore(th,head.cells[2]||null)}}
...
function init(){hideStorageDuplicates();hookFetch();watch('weekHeat',decorateHeatmaps);watch('monthWeekHeat',decorateHeatmaps);watch('yearMonthHeat',decorateHeatmaps);watch('watchdogRows',decorateWatchdog);setTimeout(()=>{hideStorageDuplicates();decorateHeatmaps();decorateWatchdog()},80);setInterval(decorateHeatmaps,60000);let lang=$('languageSelect');if(lang)lang.addEventListener('change',()=>setTimeout(()=>{decorateWatchdog();decorateHeatmaps()},10))}
