...
.exportArchiveCard{grid-column:span 12}.exportArchiveMeta{margin-top:10px;font-size:.8rem;color:var(--muted)}
...
function addMeters(){let boxes=document.querySelectorAll('#device .infoBox');if(boxes.length<4)return;
 let sys=document.createElement('div');sys.className='usageBlock';boxes[0].appendChild(sys);makeMeter(sys,'meterRam',txt('RAM-Auslastung','RAM usage'));makeMeter(sys,'meterFlash',txt('Programmspeicher','Program storage'));
 let store=document.createElement('div');store.className='usageBlock';boxes[2].appendChild(store);makeMeter(store,'meterRecent',txt('Ringspeicher','Ring buffer'));makeMeter(store,'meterArchive',txt('Langzeit-Ringspeicher','Long-term ring buffer'));makeMeter(store,'meterAutark',txt('Autark-Ringspeicher','Standalone ring buffer'));
 let fs=document.createElement('div');fs.className='usageBlock';boxes[3].appendChild(fs);makeMeter(fs,'meterFs','LittleFS');}
function addArchiveExport(){let grid=document.querySelector('#export .grid');if(!grid||$('archiveExportCard'))return;let c=document.createElement('div');c.id='archiveExportCard';c.className='card exportArchiveCard';c.innerHTML='<h2>🗄 '+txt('Langzeit-Ringspeicher','Long-term ring buffer')+'</h2><p>'+txt('Vollständiger CSV-Export des Langzeitarchivs. Der Export wird direkt gestreamt und benötigt keine zusätzliche temporäre Datei im Flash.','Complete CSV export of the long-term archive. The export is streamed directly and does not require an additional temporary file in flash.')+'</p><a class="btn primary" href="/archive.csv">'+txt('Langzeit-CSV herunterladen','Download long-term CSV')+'</a><div id="archiveExportMeta" class="exportArchiveMeta"></div>';grid.appendChild(c)}
function formatStoragePair(id,a,b){let el=$(id);if(el)el.textContent=fmt(a)+' / '+fmt(b)}
function updateFromStatus(d){if(!d)return;addWeek();isoWeekFromDevice();formatStoragePair('devRecent',d.eventCount,d.ringCapacity);formatStoragePair('devArchive',d.archiveCount,d.archiveCapacity);formatStoragePair('devAutark',d.autarkCount,d.autarkCapacity);setMeter('meterRecent',d.eventCount,d.ringCapacity);setMeter('meterArchive',d.archiveCount,d.archiveCapacity);setMeter('meterAutark',d.autarkCount,d.autarkCapacity);setMeter('meterFs',d.fsUsed,d.fsTotal);let heapTotal=Number(d.heapTotal||0);setMeter('meterRam',Math.max(0,heapTotal-Number(d.heapFree||0)),heapTotal);let sketchTotal=Number(d.sketchUsed||0)+Number(d.sketchFree||0);setMeter('meterFlash',d.sketchUsed,sketchTotal);let meta=$('archiveExportMeta');if(meta)meta.textContent=txt('Belegt: ','Stored: ')+fmt(d.archiveCount)+' / '+fmt(d.archiveCapacity)}
function hookFetch(){const original=window.fetch.bind(window);window.fetch=async function(){let r=await original.apply(null,arguments);try{let u=String(arguments[0]||'');if(u.indexOf('/api/status')>=0)r.clone().json().then(updateFromStatus).catch(()=>{})}catch(e){}return r}}
function fastButton(id,delta,url){let b=$(id);if(!b)return;b.onclick=async function(){let n=$('todayCount'),old=Number((n.textContent||'0').replace(/\D/g,''))||0;if(n)n.textContent=Math.max(0,old+delta);b.classList.add('fastFeedback');setTimeout(()=>b.classList.remove('fastFeedback'),180);try{let r=await fetch(url,{method:'POST',cache:'no-store'});if(!r.ok)throw new Error(await r.text())}catch(e){if(n)n.textContent=old;let notice=$('notice');if(notice){notice.style.display='block';notice.textContent=e.message||String(e)}}}}
function init(){addWeek();addBeta();addMeters();addArchiveExport();hookTheme();hookFetch();fastButton('addBtn',1,'/api/add');fastButton('undoBtn',-1,'/api/delete-last');setInterval(isoWeekFromDevice,1000)}
init();
})();
</script>
)HTML";
